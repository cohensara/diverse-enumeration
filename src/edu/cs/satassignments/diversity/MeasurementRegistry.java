...
public class MeasurementRegistry {
    ...
    public static void removeAllMeasures() {
    	globalMeasures.clear();
    	windowMeasures.clear();
    	allWindows.clear();
    }
    ...
    public static void addModel(HashMap<Integer, Boolean> model) {
        ...
        for (Integer size: allWindows.keySet()) {
            ...
            HashMap<Integer, Boolean> removed = w.add(model);
            for (WindowMeasure wm: windowMeasures.get(size)) {
                wm.addModel(w, removed, model);
            }
        }
    }
